...
function BarcodeScanner() {
  ...
  const handleScan = (detected) => {
    if (detected && detected.length > 0) {
      setResult(detected[0].rawValue);
      // If not already scanned, show form to add product
      if (!scannedProducts.some((p) => p.code === detected[0].rawValue)) {
        setForm({
  ...
  const validate = () => {
    const errors = {};
    if (!form.productName) errors.productName = "Required";
    if (!form.mrp) errors.mrp = "Required";
    if (!form.discount) errors.discount = "Required";
  ...
  // Remove product
  const removeProduct = (code) => {
    setScannedProducts((products) => products.filter((p) => p.code !== code));
  };

  // Calculate total
  const getTotal = () => {
    return scannedProducts.reduce(
      (sum, p) => sum + (p.mrp - p.discount) * p.quantity,
      0
    );
  };
  ...
  return (
    <div className="barcode-scanner-container">
      <h2 className="barcode-scanner-title">Barcode Scanner</h2>
      ...
      {scannedProducts.length === 0 ? (
        ...
        <div className="current-bill-table-wrapper">
          <table className="current-bill-table">
            <thead>
              <tr>
                <th>Product Name</th>
                <th>MRP</th>
          ...
          <div className="bill-actions">
            ...
            <button onClick={clearBill} className="clear-bill-btn">
              Clear Bill (New Customer)
            </button>
          </div>
        </div>
      ...
      {pdfDownloaded && (
        <div className="pdf-downloaded-message">
          <span>
            Invoice PDF downloaded!{" "}
            <PDFDownloadLink
              document={
